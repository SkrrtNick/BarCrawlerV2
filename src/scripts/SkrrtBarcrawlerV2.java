...
public class SkrrtBarcrawlerV2 implements TribotScript, PaintInfo {
    ...
    public String[] getPaintInfo() {
        return new String[]{"SkrrtBarcrawler v" + version, "Time ran: " + SkrrtPaint.getRuntimeString(), "Status: " + getStatus()};
    }
    ...
    public void execute(String args) {
        Painting.addPaint(SkrrtPaint::paint);
        if (!args.isBlank()) {
            ScriptSettings settings = ScriptSettings.getDefault();
            settings.load(args, Profile.class).ifPresent(SkrrtBarcrawlerV2::setRunningProfile);
        }
